//
//  IMAnalyticsAvgCholesterolBarViewController.m
//  HealthMemoir
//
...
@interface IMAnalyticsAvgCholesterolBarViewController () <JBBarChartViewDelegate, JBBarChartViewDataSource>
...
@implementation IMAnalyticsAvgCholesterolBarViewController
...
- (NSDictionary *)parseData:(NSArray *)theData {
    ...
    for(NSString *date in dictionary) {
        if([dictionary[date] count] >= 1) {
            ...
            [formattedData addObject:@{@"date":date, @"avgCholesterol":[NSNumber numberWithDouble:readingAvg]}];
        }
    }
...
- (void)barChartView:(JBChartView *)barChartView didSelectBarAtIndex:(NSUInteger)index touchPoint:(CGPoint)touchPoint {
    ...
    NSNumber *valueNumber = [info objectForKey:@"avgCholesterol"];
    ...
    [self.informationView setTitleText:@"Avg Cholesterol"];
