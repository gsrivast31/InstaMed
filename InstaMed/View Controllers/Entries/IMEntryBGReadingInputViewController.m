//
//  IMEntryBGReadingInputViewController.m
//  InstaMed
//
...
@interface IMEntryBGReadingInputViewController ()
{
    NSString *value;
    NSString *mgValue;
    NSString *mmoValue;
...
@implementation IMEntryBGReadingInputViewController

#pragma mark - Setup
- (id)init {
    self = [super init];
    eventFilterType = BGReadingFilterType;
...
- (id)initWithEvent:(IMEvent *)theEvent {
    ...
    if(self) {
        ...
        IMBGReading *reading = (IMBGReading *)[self event];
        if(reading) {
...
- (IMEvent *)saveEvent:(NSError **)error {
    ...
    if(moc) {
        ...
        IMBGReading *reading = (IMBGReading *)[self event];
        if(!reading) {
            NSEntityDescription *entityDescription = [NSEntityDescription entityForName:@"IMBGReading" inManagedObjectContext:moc];
            reading = (IMBGReading *)[[IMBaseObject alloc] initWithEntity:entityDescription insertIntoManagedObjectContext:moc];
            reading.filterType = [NSNumber numberWithInteger:BGReadingFilterType];
