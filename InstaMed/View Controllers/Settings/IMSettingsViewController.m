...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.section == 0) {
        ...
        if(indexPath.row == 0) {
            ...
            cell.textLabel.text = [NSString stringWithFormat:@"%@ 😊", NSLocalizedString(@"Rate HealthMemoir in the App Store", nil)];
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        } else if(indexPath.row == 2) {
    ...
    if(indexPath.section == 0) {
        ...
        if(indexPath.row == 0) {
            if([MFMailComposeViewController canSendMail]) {
                ...
                [mailController setSubject:@"HealthMemoir Support"];
                [mailController setToRecipients:@[@"gaurav.sri87@gmail.com"]];
                [mailController setMessageBody:[NSString stringWithFormat:@"%@\n\n", NSLocalizedString(@"I need help with HealthMemoir! Here's the problem:", @"A default message shown to users when contacting support for help")] isHTML:NO];
                if(mailController) {
