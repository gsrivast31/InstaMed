//
//  IMMedicineInputViewController.m
//  InstaMed
//
...
@interface IMMedicineInputViewController ()
{
    BOOL isFetchingSmartInputPrediction;
}
@end

@implementation IMMedicineInputViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if(![self event] && moc && isFirstLoad)
    {
        if([[NSUserDefaults standardUserDefaults] boolForKey:kUseSmartInputKey])
        {
            ...
            // create temporary IMMedicine objects so that
            // they can be factored into the Smart Input calculation
            NSMutableArray *unsavedEntries = [NSMutableArray array];
            for(IMInputBaseViewController *vc in self.parentVC.viewControllers)
            {
                if([vc isKindOfClass:[self class]] && vc != self)
                {
                    IMMedicineInputViewController *medicineVC = (IMMedicineInputViewController *)vc;
                    if(![medicineVC event])
                    {
                        NSEntityDescription *entityDescription = [NSEntityDescription entityForName:@"IMMedicine" inManagedObjectContext:moc];
                        IMMedicine *entry = [[IMMedicine alloc] initWithEntity:entityDescription insertIntoManagedObjectContext:nil];
            ...
            [[IMEventController sharedInstance] attemptSmartInputWithExistingEntries:unsavedEntries success:^(IMMedicine *event) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    IMEventInputViewCell *cell = (IMEventInputViewCell *)[strongSelf.tableView cellForRowAtIndexPath:indexPath];
                    [[(IMCategoryInputView *)cell.control textField] becomeFirstResponder];
                });
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    IMEventInputViewCell *cell = (IMEventInputViewCell *)[strongSelf.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
...
- (void)didBecomeActive
{
    ...
    if(!isFetchingSmartInputPrediction && !self.event && !self.activeControlIndexPath)
    {
        self.activeControlIndexPath = [NSIndexPath indexPathForRow:0 inSection:0];
    }
    ...
    if(self.activeControlIndexPath)
    {
        ...
        if([cell.control isKindOfClass:[IMCategoryInputView class]])
        {
            [[(IMCategoryInputView *)cell.control textField] becomeFirstResponder];
        }
        else
