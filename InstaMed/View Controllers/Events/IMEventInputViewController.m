...
@interface IMEventInputViewController ()
{
...
// Logic
- (void)addVC:(IMInputBaseViewController *)vc;
- (void)removeVC:(IMInputBaseViewController *)vc;

// Helpers
- (IMInputBaseViewController *)activeViewController;

@end

@implementation IMEventInputViewController
...
- (void)viewDidLoad
{
    ...
    [self.collectionView registerClass:[IMEventCollectionViewCell class] forCellWithReuseIdentifier:@"cellIdentifier"];
...
{
    IMInputBaseViewController *activeViewController = [self activeViewController];
...
- (void)addVC:(IMInputBaseViewController *)aVC
{
    if(aVC)
    {
...
- (void)removeVC:(IMInputBaseViewController *)aVC
{
...
- (void)activateTargetViewController
{
    ...
    {
        IMInputBaseViewController *targetVC = self.viewControllers[newIndex];
        if(targetVC)
        {
...
{
    IMInputBaseViewController *activeViewController = [self activeViewController];
    if((self.event && [self.event.latitude doubleValue] != 0.0 && [self.event.longitude doubleValue] != 0.0) || ([activeViewController.lat doubleValue] != 0.0 && [activeViewController.lon doubleValue] != 0.0))
    {
        UIActionSheet *actionSheet = nil;
        actionSheet = [[UIActionSheet alloc] initWithTitle:nil
                                                  delegate:activeViewController
...
{
    IMEventCollectionViewCell *cell = (IMEventCollectionViewCell *)[aCollectionView dequeueReusableCellWithReuseIdentifier:@"cellIdentifier" forIndexPath:indexPath];
    
    IMInputBaseViewController *vc = self.viewControllers[indexPath.row];
