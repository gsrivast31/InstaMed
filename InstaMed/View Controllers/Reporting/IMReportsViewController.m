...
#import "IMCarbsChartViewController.h"

@interface IMReportsViewController ()
{
    NSArray *reports;
    NSArray *reportData;
    ...
    IMDateButton *fromDateButton, *toDateButton;
...
@implementation IMReportsViewController
...
- (void)viewDidLoad
{
    ...
    reports = @[
                @{@"title": NSLocalizedString(@"Blood Glucose Readings", nil), @"description": NSLocalizedString(@"A line chart showing your blood glucose and general trend over a given period", nil), @"class": [IMGlucoseLineChartViewController class]},
                @{@"title": NSLocalizedString(@"Time-of-Day Glucose Readings", nil), @"description": NSLocalizedString(@"A scatter chart showing blood glucose levels during different time segments", nil), @"class": [IMGlucoseTimeOfDayChartViewController class]},
                @{@"title": NSLocalizedString(@"Daily Blood Glucose Ranges", nil), @"description": NSLocalizedString(@"A candlestick chart showing your first, last, lowest and highest glucose readings per day", nil), @"class": [IMGlucoseCandlestickChartViewController class]},
                @{@"title": NSLocalizedString(@"Carbohydrate in-take", nil), @"description": NSLocalizedString(@"A stacked bar chart (segmented by morning, afternoon and evening) showing total carbohydrate in-take per day", nil), @"class": [IMCarbsChartViewController class]},
                @{@"title": NSLocalizedString(@"Healthy Glucose Tally", nil), @"description": NSLocalizedString(@"A pie chart showing the number of healthy glucose readings versus unhealthy over a given period", nil), @"class": [IMGlucoseDonutChartViewController class]}
                ];
    ...
    dateRangeLabel.font = [IMFont standardDemiBoldFontWithSize:14.0f];
    ...
    dateRangeToLabel.font = [IMFont standardDemiBoldFontWithSize:14.0f];
    dateRangeToLabel.text = NSLocalizedString(@"and", nil);
    [self.view addSubview:dateRangeToLabel];
    
    fromDateButton = [[IMDateButton alloc] initWithFrame:CGRectMake(10.0f, y-6.0f, 100.0f, 30.0f)];
    ...
    toDateButton = [[IMDateButton alloc] initWithFrame:CGRectMake(10.0f, y-6.0f, 100.0f, 30.0f)];
    ...
    for(NSInteger i = 0; i < [reports count]; i++)
    {
        ...
        IMReportPreviewView *reportPreview = [[IMReportPreviewView alloc] initWithFrame:CGRectZero andInfo:info];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if(chartInfo)
    {
        ...
        IMChartViewController *chartVC = [(IMChartViewController *)[chartClass alloc] initWithData:reportData];
...
{
    IMDatePickerController *datePicker = [[IMDatePickerController alloc] initWithFrame:self.view.bounds andDate:(sender.tag == 0 ? fromDate : toDate)];
...
#pragma mark - IMDatePickerDelegate methods
