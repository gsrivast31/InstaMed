...
@interface IMIntroductionTooltipView ()
{
...
@implementation IMIntroductionTooltipView
...
- (UIView *)pageForIndex:(NSInteger)index
{
    ...
    {
        header.text = NSLocalizedString(@"Hi there!", nil);
        content.text = NSLocalizedString(@"InstaMed is a new kind of diabetic journal that lets you track your blood glucose, medication, food and personal activities.\n\nTo learn more swipe your finger to the left.", nil);
    }
    else if(index == 1)
    {
        header.text = NSLocalizedString(@"Nice to meet you", nil);
        content.text = NSLocalizedString(@"InstaMed makes keeping your journal up-to-date as easy as possible by analysing your habits.\n\nIt may take a few days to really get to know you, so please be patient!", nil);
    }
    else if(index == 2)
    {
        header.text = NSLocalizedString(@"Stay safe", nil);
        content.text = NSLocalizedString(@"InstaMed cannot and will not advise you with regards to medical care.\n\nIf you have questions or concerns regarding the state of your health please see a medical professional.", nil);
    }
